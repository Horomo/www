...
const CLASSIFICATION_META: Record<UsefulElementResult['classification'], { label: string; tone: 'gold' | 'cyan' | 'violet' | 'pink' }> = {
  strong: { label: '身強 · Strong', tone: 'gold' },
  weak: { label: '身弱 · Weak', tone: 'cyan' },
  borderline: { label: '中和 · Borderline', tone: 'violet' },
  special_structure: { label: '從格-suspect · Special', tone: 'pink' },
};

const STRUCTURE_ZH: Record<string, string> = {
  companion: '比劫', resource: '印', output: '食伤', wealth: '財', influence: '官杀',
};

export function UsefulElementCard({ useful }: { useful: UsefulElementResult }) {
  const meta = CLASSIFICATION_META[useful.classification];
  const asserted = useful.usefulElement !== null;
  const el = (key: string) => `${EL_LABEL[key].en} · ${EL_LABEL[key].zh}`;
  const cfg = useful.config;
  const pct = Math.max(0, Math.min(100, Math.round(useful.strengthRatio * 100)));
  const tile = 'rounded-[1.4rem] bg-[linear-gradient(135deg,rgba(255,255,255,0.78),rgba(255,255,255,0.54))] p-4 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.64)]';

  return (
    <GlowCard accent="violet" className="p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Badge tone="violet">Strength engine</Badge>
          <h3 className="mt-3 font-serif text-[1.8rem] text-[#151d22]">Useful Element · 用神</h3>
          <p className="mt-2 max-w-xl text-sm leading-7 text-[#151d22]/66">
            A deterministic, weighted reading of Day Master strength. The weights below are Horomo&apos;s tunable stance, not a universal standard.
          </p>
        </div>
        <div className="rounded-[1.5rem] bg-[linear-gradient(135deg,rgba(124,58,237,0.14),rgba(255,255,255,0.76))] px-4 py-3 text-right shadow-[inset_0_0_0_1px_rgba(124,58,237,0.16)]">
          <div className="text-xs uppercase tracking-[0.24em] text-[#874e58]/62">Classification</div>
          <div className="mt-2 text-xl font-semibold text-[#151d22]">{meta.label}</div>
          <div className="mt-1 text-xs text-[#151d22]/56">Support {pct}% · ratio {useful.strengthRatio}</div>
        </div>
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-3">
        <div className={tile}>
          <div className="text-[11px] uppercase tracking-[0.18em] text-[#151d22]/48">Useful Element (用神)</div>
          <div className={cn('mt-2 text-lg font-semibold', asserted ? 'text-[#7c3aed]' : 'text-[#874e58]')}>
            {asserted ? el(useful.usefulElement as string) : 'Not asserted · complex chart'}
          </div>
        </div>
        <div className={tile}>
          <div className="text-[11px] uppercase tracking-[0.18em] text-[#151d22]/48">Favorable (喜神)</div>
          <div className="mt-2 text-sm font-medium text-[#006a62]">{asserted ? useful.favorableElements.map(el).join(', ') : '—'}</div>
        </div>
        <div className={tile}>
          <div className="text-[11px] uppercase tracking-[0.18em] text-[#151d22]/48">Unfavorable (忌神)</div>
          <div className="mt-2 text-sm font-medium text-[#874e58]">{asserted ? useful.unfavorableElements.map(el).join(', ') : '—'}</div>
        </div>
      </div>

      <div className="mt-5 rounded-[1.4rem] bg-[linear-gradient(135deg,rgba(255,255,255,0.74),rgba(255,255,255,0.5))] p-4 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.64)]">
        <div className="flex flex-wrap items-baseline justify-between gap-2 text-xs text-[#151d22]/60">
          <span>Support (扶) <span className="font-semibold text-[#006a62]">{useful.supportScore}</span> vs Drain (抑) <span className="font-semibold text-[#874e58]">{useful.drainScore}</span></span>
          <span>weak ≤ {cfg.weakThreshold} · strong ≥ {cfg.strongThreshold} · special ≥ {cfg.extremeThreshold}</span>
        </div>
        <div className="mt-3 h-2.5 w-full overflow-hidden rounded-full bg-[#874e58]/20">
          <div className="h-full rounded-full bg-[linear-gradient(90deg,#006a62,#40e0d0)]" style={{ width: `${pct}%` }} />
        </div>
        <div className="mt-3 text-xs leading-6 text-[#151d22]/56">
          Weighted by structure — 比劫 {useful.structureScores.companion}, 印 {useful.structureScores.resource}, 食伤 {useful.structureScores.output}, 財 {useful.structureScores.wealth}, 官杀 {useful.structureScores.influence}.
        </div>
      </div>

      <div className="cosmic-scrollbar mt-5 overflow-x-auto">
        <table className="min-w-[560px] w-full text-sm">
          <thead>
            <tr className="text-xs uppercase tracking-[0.18em] text-[#151d22]/44">
              <th className="py-2 pr-4 text-left">Position</th>
              <th className="py-2 px-2 text-center">Stem</th>
              <th className="py-2 px-2 text-center">Ten God</th>
              <th className="py-2 px-2 text-center">Side</th>
              <th className="py-2 px-2 text-right">Weight</th>
            </tr>
          </thead>
          <tbody>
            {useful.breakdown.map((c, i) => (
              <tr key={`${c.position}-${i}`} className="border-t border-white/30 text-[#151d22]/74">
                <td className="py-2 pr-4">{c.position}{c.monthCommand ? <span className="ml-1 text-xs text-[#7c3aed]">· 月令</span> : null}</td>
                <td className="py-2 px-2 text-center font-zh font-semibold">{c.stem} <span className="text-xs text-[#151d22]/50">{EL_LABEL[c.element].en}</span></td>
                <td className="py-2 px-2 text-center">{c.tenGod} <span className="text-xs text-[#151d22]/44">({STRUCTURE_ZH[c.structure]})</span></td>
                <td className={cn('py-2 px-2 text-center text-xs font-medium', c.side === 'support' ? 'text-[#006a62]' : 'text-[#874e58]')}>{c.side === 'support' ? 'support 扶' : 'drain 抑'}</td>
                <td className="py-2 px-2 text-right tabular-nums">{c.weight}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="mt-4 rounded-[1.4rem] bg-[linear-gradient(135deg,rgba(124,58,237,0.1),rgba(255,255,255,0.6))] px-4 py-3 text-sm leading-7 text-[#151d22]/72 shadow-[inset_0_0_0_1px_rgba(124,58,237,0.12)]">
        {useful.reasoning}
      </p>

      <div className="mt-3 text-xs leading-6 text-[#151d22]/48">
        Weights used — 月令: {cfg.monthCommandWeight}, other branch: {cfg.branchWeight}, stem: {cfg.stemWeight}, roots 本/中/余: {cfg.rootWeight.primary}/{cfg.rootWeight.middle}/{cfg.rootWeight.residual}. Calibrate these in the engine config; the breakdown above shows where each point comes from.
      </div>
    </GlowCard>
  );
}
...
export default function BaziResultView({
  ...
  // Single deterministic engine call (same function the AI route uses) — not a re-implementation.
  const usefulElement = useMemo(
    () => computeUsefulElement(result.pillars, result.pillars.day.stemIdx, result.unknownTime),
    [result],
  );
  ...
  return (
    <div key={resultAnchor} className="space-y-10 animate-reveal">
      ...
      <SectionFrame eyebrow="Support Matrix" title="Analysis Modules" description="Supporting systems are grouped here: structural balance, Ten Gods distribution, true solar correction, and the raw pillar matrix used for close reading." accent="violet">
        ...
        <UsefulElementCard useful={usefulElement} />
