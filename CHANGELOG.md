...
### Added
- `feat: MCP compute_compatibility (合婚) — deterministic two-chart compatibility tool`
  New read-only MCP tool that takes two birth inputs (each validated exactly like the single-chart tools: date/time format, IANA timezone, longitude range, 60° longitude/timezone guard — errors are prefixed `personA:`/`personB:`) and analyzes compatibility on four rule-based axes, each 0–10 with a reasoning string: (1) Day Master 生/克 relation (天干生克, fixed scores 生 8 / 同 7 / 克 4, direction reported), (2) all cross-chart Earthly Branch pairs checked against hardcoded standard tables for 六合/半三合/半三会/冲/刑/害/破 (5 neutral ± documented per-interaction deltas), (3) 用神 complementarity (flat count of each person's needed element in the partner's chart; a borderline/從格 chart whose 用神 the engine refuses to assert makes that direction "cannot assess" — never guessed), and (4) spouse star 配偶星 (male → 財, female → 官杀), asserted only when BOTH genders are provided. Overall = weighted mean (Day Master 0.2, branches 0.35, 用神 0.3, spouse star 0.15; unassessable axes dropped and weights renormalized — weights are documented in code and echoed in the output). Both charts run through the SAME `resolveChart` → `computeBazi`/`computeUsefulElement` pipeline as the existing tools — no pillar or 用神 logic re-derived, no engine changes, fully deterministic (same input → same output). Golden-tested against a known couple (金生水 Day Masters, 辰酉六合, both charts lacking fire).
